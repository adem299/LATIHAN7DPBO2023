...
public class Game extends Canvas implements Runnable
{
    ...
    public Game()
    {
        try
        {
            ...
            if(running)
            {
                ...
                handler.add(new Target(random.nextInt(640), random.nextInt(480))); // random postion for target object
            }
        } catch(Exception e)
        {
    ...
    {
        this.score += score; // this score increased by score value (+1)
    }
    
    public void setTouch(int touchscore){
        this.touch += touchscore;
    }
    ...
    public void render()
    {
        ...
        if(bs == null)
        {
            this.createBufferStrategy(4); // for SOMEBITS error
            return;
        }
        ...
        if(running == true)
        {
            ...
            Font nFont = oldFont.deriveFont(oldFont.getSize() * 1.3f);
            g.setFont(nFont);
            
            g.setColor(Color.ORANGE);
            g.drawString("TouchScore : " + Integer.toString(touch), 20, 50);
        ...
        {   
            ...
            for(int i = 0; i < handler.count(); i++)
            {
                ...
                if(handler.get(i).getType().equals("Target"))
                {
                    ...
                    // collusion (player and target) left top right bottom
                    if((player.getX()+player.getWidth() >= target.getX()) && (player.getY()+player.getHeight() >= target.getY())
                            && (player.getX() <= target.getX()+target.getWidth()) && (player.getY() <= target.getY()+target.getHeight())) {
                        handler.remove(i); // remove target object
                        handler.add(new Target(random.nextInt(640), random.nextInt(480))); // and new target object in other random positon
                        setTouch(+1); // touch counter
                    }
                }
            }
        }
